		...
		\begin{subfigure}{1.1\columnwidth}
			...
			\parbox{.8\columnwidth}{\hspace{1cm}
				\scalebox{.9}{
					...
					\end{scope}
					\begin{scope}[yshift=-3.8cm,grow'=up, frontier/.style={distance from root=13mm}, ->]
					\Tree [.\node [circle] (rootd) {};
...
We conduct four types of experiments to validate the feasibility of our approach. Showing:
(1) semantic annotation can be consistently applied to 
LL, through inter-annotator agreement (IAA) experiments.
(2) a valid corrector scores high on this measure.
...
\textcolor{red}{
We also checked the sensibility of USim to single edit appliance by type as found in the NUCLE references. For each annotation and source we sampled an order for applying the edits sequentially and chose the source randomly to be one of the resulting sentences. We then compared the difference in score before and after applying each edit and averaged those deltas by the error type applied finding USim returns an exact zero for most of the types with solely "unclear meaning" getting any delta at all on average.% The diff is 0.0327868852 if it matters
\lc{perhaps remove this self doubt?>>>}
 A contributing factor to that might be the fact that many of those corrections consist of more than a couple of words, while in other types of correction long changes are less frequent. Still, it is interesting to see that the places where USim is not zero is on the sole type of correction where annotator might change meaning.
}
