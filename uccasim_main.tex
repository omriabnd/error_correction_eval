...
a number of proposals for new, improved reference-based GEC measures \cite{felice2015towards,napoles2015ground,dahlmeier2012better}\oa{there are also others, no? also, don't they need to be sorted by year of publication?}, as well as the compilation of multiple references for each input.

Nevertheless, the size and heterogeneity of the space of valid outputs for a sentence often prohibits
obtaining a reference set that covers this space well, thereby limiting the applicability
of reference-based measures \cite{bryant2015far}.
To address this we propose a semantic reference-less measure, {\sc USim}.
...
{\sc USim} is conceptually related to reference-less measures developed
for machine translation
\cite{reeder2006measuring,albrecht2007regression,specia2009estimating,specia2010machine}.
Notably, XMEANT \cite{lo2014xmeant} compares the source sentence to the output
...
We conduct experiments to confirm the validity of {\sc USim}.
Specifically, we show that 
(1) UCCA can be consistently and automatically applied to learner language, 
...
Our experiments also indicate that UCCA parsing technology is already sufficiently mature for
...
procede to defining {\sc USim}, which compares two UCCA structures
...
We train two UCCA annotators by annotating both learner language and standard English
...
some\oa{instead of ``some'' write how many}
of which were annotated by both annotators.
...
As the state-of-the-art systems are over-conservative,
they rarely alter the source sentences sufficiently to yield semantically unfaithful outputs,
and are hence not useful for our purposes.\footnote{We demonstrate this in a paper currently under review.
  Due to the same reason, available human judgments of corrections produced by these systems are not
  applicable for evaluating {\sc USim}.}
...
for the system that obtained the lowest GLUE\oa{add ref} score.
