...
The resulting $F$-score, using $A$ is called UCCA Similarity ({\sc USim}).
...
as, unlike DAG $F$-score, it also aligns nodes whose yields are not in valid alignment with one another.
...
\textcolor{red}{
We also checked the sensibility of USim to different edit types. For each NUCLE sentence and set of edits, we sample an order in which the edits are applied sequentially and select the source randomly to be one of the resulting sentences. We then compared the difference in {\sc USim} before and after applying each edit and averaged those differences by the applied edit type. We find that the difference are $<10^{-5}$ for most of the types, where only "unclear meaning" obtains obtains a positive delta (0.033).% The diff is 0.0327868852 if it matters
}
