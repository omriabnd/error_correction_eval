...
\begin{abstract}
  ...
  thereby complementing existing reference-less measures for measuring the output's grammaticality.
...
a number of proposals for new, improved reference-based GEC measures \cite{felice2015towards,napoles2015ground,dahlmeier2012better}\oa{there are also others, no? also, don't they need to be sorted by year of publication?}, as well as the compilation of
multiple references for each input.

%%% until here
Nevertheless, reference-based evaluation are inherently limited by the number and heterogeneity of valid outputs,
which prohiobtaining a set of references that would well-represent the space of valid corrections is likely 
to be prohibitively difficult \cite{bryant2015far}.
We thus propose {\sc USim}, a semantic reference-less evaluation measure which assesses the extent to which the semantic
representation of the source resembles that of the output.
Our proposal complements the reference-less measure proposed recently by 
\newcite{napoles-sakaguchi-tetreault:2016:EMNLP2016}, which uses grammatical error detection
techniques to assess the grammaticality of the output.
...
Of which, perhaps the most closely related approach to ours is taken by MEANT \cite{lo2011meant}, 
which is based on a comparison of semantic role labeling structures on the source and target.
Our use of UCCA is motivated by its wider coverage of predicate types, as opposed to 
MEANT's focus on verbal argument structures. See \cite{birch2016hume} for discussion. 
...
However, the measure can be easily adapted to use other semantic schemes, such as AMR \cite{}.
...
We conduct a set of experiments using both a human and an automatic version of {\sc USim} that 
confirm its validity. Specifically we show that 
(1) semantic structural annotation can be consistently and automatically applied to learner language, 
...
Our experiments further indicates that UCCA parsing technology is already sufficiently mature for
an automatic variant of {\sc USim} to provide reliable results.
...
or of any other known language. This entails difficulties in defining syntactic annotation for learner language, 
as, on the face of it, the language of each learner has to be annotated in its own terms.

Syntactic schemes for learner language annotate syntactically erroneous sentences in different ways.
...
%A reference-less approach to the complementary aspect of the evaluation, namely grammaticality,
%has been tackled using Grammatical Error Detection \cite{napoles-sakaguchi-tetreault:2016:EMNLP2016}.

%In this section we present the {\sc USim} measure for capturing faithfulness
%through the graph similarity of the UCCA representations of the source and the output.
...
%semantically similar sentences, formulated by different learners, may use considerably different structures
...
We begin by defining a simplified measure that compares two UCCA annotations over the 
same set of tokens, which is used to measure inter-annotator agreement (IAA). We then
procede to defining the {\sc USim} measure for comparing two UCCA structures over
different tokens.
...
learner language, through inter annotator agreement (IAA) experiments.
...
These results suggest that annotating learner language with UCCA does not degrade IAA, 
and can be applied as consistently to learner language as to standard English.

Table \ref{tab:Distances} (left-hand side) presents the {\sc USim} scores obtained by comparing 
the NUCLE references and the source, or equivalently the score of a perfect correction.
...
Our results indicate that a perfect correction obtains a score comparable
...
We turn to experimenting with an automatic variant of USim, where UCCA
...
We obtain a {\sc USim} score of 0.7 between the automatically generated parses of the reference
