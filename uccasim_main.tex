...
We conduct four types of experiments to validate {\sc USim}, showing:
...
We find that the difference are negligible ($<10^{-5}$) for most of the types, 
where only correcting the ``unclear meaning'' type changes {\sc USim}'s score (the difference is +0.033).
% The diff is 0.0327868852 if it matters
...
Indeed, automatic USim obtains scores of 0.32-0.39 for 4 of the systems, and 0.19
for the system that obtained the lowest GLEU \cite{napoles2015ground} score.
For completeness, we run {\sc USim} on the 4 references provided for each
source by JFLEG and obtain scores of 0.72-0.78.
