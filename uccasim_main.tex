		...
		\begin{subfigure}{1.1\columnwidth}
			...
			\parbox{.8\columnwidth}{\hspace{1cm}
				\scalebox{.9}{
					...
					\Tree [.\node [circle] (rootd) {};
					\edge node [auto=left]{\scriptsize A}; \node (Hed) {He};
					\edge node[auto=right]{\scriptsize P}; \node (gave) {gave};
					\edge node[auto=right]{\scriptsize A};\node (John) {John};
					\edge node[auto=right]{\scriptsize A};
					[.\node [circle] (an appled) {};
					\edge node[auto=left]{\scriptsize E}; \node (and) {an};
					\edge node[auto=right]{\scriptsize C}; \node (appled) {apple};
					]
]
					\end{scope}
					\begin{scope}[dashed]
			...
			\end{subfigure}
				...
				\caption{\label{fig:example}
					UCCA structures of a learner language (top) and correction (bottom) including word alignments (dashed).
				}
			\end{figure}
...
$A \subset V_s \times V_c$. See Figure \ref{fig:example}. 
