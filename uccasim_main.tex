...
We find that differences are neglible ($<10^{-5}$) for most types, 
where only correcting errors of type ``unclear meaning'' changes {\sc USim}'s score (the difference is +0.033).
...
other as they are to their gold standard parse. This indicates 
...
source  and obtain scores of 0.72-0.78.
...
We believe that using RBMs in conjunction with RLMs in the training and development of GEC
