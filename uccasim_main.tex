...
\begin{abstract}
  ...
  We show that ungrammatical errors are well defined in the UCCA \cite{abend2013universal} scheme, that they are captured by recent parsers as well as grammatical ones do, and that {\sc UCCASim} is high for meaning preserving corrections and low for meaning altering ones.
\end{abstract}
...
Semantic annotation schemes are also gaining support and proven to be useful for many tasks \lc{have preference on what to cite?}. One desired feature of semantic annotation is its robustness to modifications that do not change the meaning such as paraphrasing \cite{abend2013universal}, and translation \cite{sulem2015conceptual}. Semantic evaluation measures were also shown to work well for other fields \cite{birch2016hume}\lc{have preference on what to cite?}.
...
proposed by \newcite{napoles-sakaguchi-tetreault:2016:EMNLP2016} should be used to incentivize corrections to correct.

Our experiments support the feasibility of the proposed approach,
by showing that (1) semantic structural annotation can be consistently and automatically applied to learner language, (2) the proposed measure is not prone to unduly penalize valid corrections and (3) the measure does penalize corrections that alter the semantic structure significantly.
...
Results also suggest that an improvement in parsing performance may further improve these scores.
