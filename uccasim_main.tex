...
\title{Faithfulness reference-less measure for grammatical error correction}
%\author{
...
\begin{abstract}
  We propose {\sc UCCASim}, a semantic measure that captures meaning preservation. Working in the context of Grammatical Error Correction (GEC) the measure compares the semantic structure of the origin and the correction without relying on human annotation or a gold standard. The measure could be used with a complementing grammatical reference-less evaluation measure to create a measure balancing between the goal of correcting grammatical mistakes and the goal of conveying the original meaning.
  We show that UCCA \cite{abend2013universal} scheme is well defined in ungrammatical sentences, that domain adaption for parsers comes freely, and that {\sc UCCASim} is high for meaning preserving corrections and low for meaning altering ones.
\end{abstract}

\section{Introduction}
Evaluation in Monolingual Translation \cite{xu2015problems,inderjeet2009summarization} and in particular in Grammatical Error Correction \cite{tetreault2008native,madnani2011they,bryant2015far,chodorow2012problems} has received much attention over the 
years. Also, some new evaluation measures for GEC were suggested \cite{felice2015towards,napoles2015ground,dahlmeier2012better} improving the ways in which references are used to evaluate. 
...
by showing: (1) semantic structural annotation can be consistently and automatically applied to learner language, (2) {\sc UCCASim} is not prone to unduly penalize valid corrections and (3) {\sc UCCASim} does penalize corrections that alter the semantic structure significantly.
...
In this section, we propose the first semantic measure that eschews the use
of reference corrections, instead measuring the semantic faithfulness of the proposed
correction to the source.
Concretely, we propose to measure the semantic similarity of the source and the proposed correction through the graph similarity of their representations.
Such a measure has to be complemented with an
error detection procedure \cite{napoles-sakaguchi-tetreault:2016:EMNLP2016}, as it only captures faithfulness, the extent to which
...
Note that {\sc UCCASim} is somewhat more relaxed than {\it DAG $F$-score},
...
presents the {\sc UCCASim} scores obtained by comparing the NUCLE references and the source, or equivalently the score of a perfect corrector.
...
we also report the {\sc UCCASim} IAA on source sentences. 
...
We obtain a UCCASim of 0.7 between the automatically generated parses of the reference
...
apply to automatic faithfulness measures, such as UCCASim.
...
of a fairly low quality indeed receive a much lower UCCASim faithfulness score. As the state-of-the-art systems are over-conservative\footnote{Paper under review. Undisclosed to preserve anonymity.}, they produce few semantically unfaithful sentences, and hence are not useful to our cause\footnote{For the same reason available human judgments do not provide meaningful information about UCCASim's ranking, but only about the grammatical measure used with it.}. We 
...
For completeness, we ran {\sc UCCASim} on the 4 references provided for each
...
{\sc UCCASim} assigns the reference 0.71 and only 0.33 to the corrector.
Moreover, although the reference makes more word changes than the proposed correction,
it still obtains a higher UCCASim score.
