...
In the field of machine translation (MT) it was shown multiple references are better for evaluation, but costly \cite{albrecht-hwa:2008:WMT,turian2006evaluation}, what led to a line of works creating semantic inspired reference-less measures \cite{reeder2006measuring,albrecht2007regression,specia2009estimating,specia2010machine}. This was followed by works to estimate translation quality in terms of its adequacy (similar to semantic faithfulness) and fluency (similar to
grammaticality)
\newcite{banchs2015adequacy}.
Perhaps the most similar approach to ours was taken by the MEANT measure \newcite{lo2011meant}, which is based on a comparison of semantic role labeling structures on the source and target.
Our decision to use UCCA in our experiments is motivated by its wider coverage of predicate types (as opposed to a focus only on verbal structures in MEANT). An elaborate comparison of SRL-based and UCCA-based approaches can be found in \newcite{birch2016hume}. We make no claim of course that UCCA is the best or the only semantic representation that can be used for defining a semantic faithfulness measure. Future work will experiment with a wider variety of semantic representations, both symbolic and distributional (e.g., \cite{cheng2015syntax}.)
