...
suffer from over-conservatism: they tend to make too few changes to the source. { \color{red} Assuming correctors are able to correct a large amount of the corrections in the corpus, this is an indication of some hidden, cross-systems bias.}
\subsection{Notation}
...
More specifically, reference-less machine translation evaluation methods were proposed and shown at the time to be reliable measures \cite{reeder2006measuring,albrecht2007regression,specia2009estimating,specia2010machine}. Perhaps most similar is the work of \newcite{banchs2015adequacy},
combining semantic and grammatical measures. { \color{red}Despite their similarity, those methods do not use up-to-date semantic annotation schemes, if at all. An exception for that is \newcite{lo2011meant} that is based on semantic role labeling, we chose to use a UCCA based measure to gain its different benefits, for more on that see \newcite{birch2016hume}. This is by no means to say that other semantic measures are not useful for the task, on the contrary, it would be an interesting future work to compare different methods beyond this proof of concept, such as those mentioned or paraphrase identification methods \cite{cheng2015syntax}.}
