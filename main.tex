...
\begin{figure}	\label{fig:corrections_dist}
	\includegraphics[width = 8cm]{exact_dists_plot}
	\caption{Frequency by rank of 2 corrections distribution. The distributions have steps containing many corrections that are just as probable with very low probability. }
\end{figure}
...
\begin{table}[h!]
	...
	\begin{tabular}{c|c|c|c|c}
		...
		\\
		\end{tabular}
		\caption{Different UCCA distance measures between LL and corrected parallel paragraphs. The scores are quite similar to the IAA, which means distances are indeed low.}
	\end{table}
...
\begin{table}[]
	\centering
	\label{tab:annotated-paragraphs}
	\begin{tabular}{lll}
		...
		1         & 10 & learner  
	\end{tabular}
	\caption{The list of paragraphs annotated, showing which annotator annotated it, which type of language is used in it and the corresponding id in the NUS corpus. Note that parallel paragraphs have the same id.}
\end{table}
\end{document}
