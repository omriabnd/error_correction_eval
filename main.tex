...
in their semantic structures. We define a measure using the universal conceptual cognitive annotation scheme (UCCA) \cite{abend2013universal} as a
...
\paragraph{$F$} We use bias-corrected and accelerated bootstrap \cite{efron1987better}, to assess the $95\%$ confidence interval of different correctors with the current NUS test data ($m=2$), based on the same process as in \ref{subsec:Assessment-values}. Results can be seen in \ref{fig:F_correctors}. The results supports our method of assessing as the bounds indeed cover the actual scores seen in all of the cases.
...
applied to LL, as LL consists of many grammatical mistakes that makes syntactic analysis ill-defined for the task. We show evidence that this is the case, by having two annotators annotate a sub-corpus from the NUS dataset, and by measuring their inter-annotator agreement.
...
\subsection{Experimental Setup.}
 For all of the experiments we used paragraphs from NUS\cite{dahlmeier2013building} consisting of about 800 UCCA nodes and about 400 words each. The paragraphs were arbitrarily chosen from the first paragraphs in NUS.
...
language. \lc{things were cut from here make sure it is fluent} Over the uncoordinated paragraphs
...
We got an F1 score for the IAA of 0.845 with
...
score for IAA (0.849) corrected paragraphs
suggests the same.

\paragraph{Is Semantic Structure Preserved in Correction?}

After showing we can really on the UCCA annotation for LL, the next experiment aims to check our hypothesis that indeed when correcting we expect faithfulness.
...
While planning the experimental setup a small problem occurred,
...
\begin{table}[]
	...
	\begin{tabular}{lll}
		...
		1         & 10 & learner  
	\end{tabular}
	\caption{The list of paragraphs annotated, showing which annotator annotated it, which type of language is used in it and the corresponding id in the NUS corpus. Note that parallel paragraphs have the same id.\label{tab:annotated-paragraphs}}
\end{table}
\end{document}
