...
not correcting an ungrammatical sentence \lc{(i.e., $P > R$) This paragraph is true without the P>R that I deleted. But is it enough without the formulation? This claim is far from trivial. In the current formulation it does not relate only to $P>R$ but also to added penalties for the possibility of correcting wrongly(in addition to the chance that it loses a bonus for wrong detection)}, condition \ref{eq:reward} is even more
...
sentences, we elicited 50 corrections by Amazon Mechanical Turk workers, allowing for reliable estimation of the distributions. To correct grammaticality and
...
Returning to condition \ref{eq:reward}, we find that the coverage
