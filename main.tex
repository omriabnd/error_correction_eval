...
\com{migrated paragraphs:
	...
	Choosing how to balance is dependent on the goals of the one collecting data, and affects the mean value as well, as discussed i g\pre{subsec:Assessment-values}. Thus, we will not discuss this issue hecision for correcting, w correcting, as ms have been studied in various fields such as genetics\cite{inita2010optimal}.
	...
	for both a smart allocation of resources when building as corpus and for assessing on given corporarections would most likely result in high loss, even when their two correctors areioctually ifferent.                                                                                              \oa{discuss RoRo higher than perfect scorer; explain why perfect is not necessarily
  ...
  due to the small value of $MN$.                               at the end explain what other factors may contribute to this difference}
...
may have considerably different structures. \newcite{nagataphrase} take an opposite approach, and attemptsto be faithful to the syntax intended by the learner. However, their
...
%In our experiments we show that a perfect corrector receives a high faithfulness score using the suggested measures.
%Unlike the former measure it requires annotation for the source and no references at all,
%solving a lot of the problem of references.
