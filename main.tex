...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		process. We shall consider any evaluation which scores a wrong correction lower than no correction at all (e.g. $F$-score). For simplicity lets consider a scenario with only one sentence. We denote by $r$ the reward for a valid correction, $p$ the penalty for a wrong correction and 0 otherwise. Thus, an average the score of a correction would be $p\left(\right)$ \lc{finish argument}
		...
		This corrector may take into account the probabilities of corrections to be in the gold standard. When choosing to correct such a corrector will choose the most frequent correction yielding on average . Notably, when on average the best correction is penalized enough to be less valuable than the reward of not producing a wrong correction, the perfect corrector will choose \emph{not} to correct.
