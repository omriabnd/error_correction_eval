...
Evaluation is especially difficult due to the fact that an ungrammatical sentence may have multiple valid
corrections, each equally good.

An important component in the evaluation of GEC is systems is their ability not only to correct the grammatical
errors of the source, but to do so in a way that is faithful to meaning of the source.
Attempts to capture this principle formally have either advocated the use of $F_{0.5}$,
where Precision is favored over Recall, and \citet{brockett2006correcting}, who suggested evaluating against two reference corrections instead of one\oa{but have they advocated conservativism or simply pointed out that one reference is not enough?}. %%%% UNTIL HERE%%%
The pursue of being conservative in that manner might bring a daunting outcome, being too formally conservative. In simpler words, not correcting errors.
...
For assessing semantic conservatism we discuss two questions. Can it induce a well-defined structure on learner language(LL)? Are corrections in fact semantically conservative? For the latter, we need to verify that the corrections make few (if any) semantic changes. our results indicate that this is the case: the corrections are similar in structure to the source. Motivated by the recent MT semantic evaluation HUME(\cite{birch2016hume}) we use UCCA(\cite{abend2013universal}) for semantic representation as a test case.
