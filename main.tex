...
There were different attempts to formally capture this precision/recall asymmetry such as the standardized use of $F_{0.5}$ over $F_{1}$, where Precision is
emphasized over Recall \cite{dahlmeier2012better} and the choices of weights in I-measure\cite{felice2015towards}.
...
by showing that (1) semantic structural annotation can be consistently applied to learner's language (LL), and (2) that the proposed measure is less prone to unduly penalize valid corrections.
%
%
...
Let $X$ be the evaluated set of source LL sentences where $X$ consists of the sentences $x_{1}\ldots x_N$, each independently sampled from some distribution $\mathcal{L}$ over LL sentences and {\color{red}denote $\mathcal{D}_{x_i}\coloneqq \mathcal{D}_{i}$.
\com{For each $x_i$, denote with $\mathcal{D}_{i}$ its distribution of corrections}}.
...
We discuss the implication of our results in \S \ref{subsec:mult_discussion}, concluding that reference-based
...
a valid correction for it will be found among $M$ randomly sampled references.\lc{we define it twice, should we delete it from the notations?}

We will now assume that the corrector may either choose not to correct the phrase,
or choose to correct it with the correction it finds the most likely.\lc{not sure I understand, what other options are there? or could less likely be more probable to be covered?} If it selected not to correct the sentence, its probability of being rewarded (i.e., producing
an output in the reference set $Y$) is $(1-p_{correct})$\lc{maybe I don't understand, didn't you meant $(1-p_{detect})$?}. Otherwise, its probability
...
a corrector is disincentivized from predicting a correction.
...
not correcting an ungrammatical sentence \lc{(i.e., $P > R$) This paragraph is true without the P>R that I deleted. But is it enough without the formulation? This claim is far from trivial. In the current formulation it does not relate to $P$ but to splitting $P$ into two (wrong corrections, and not corrected when needed), or equivalently thinking of not correcting as score $0$ which creates the formula $p_{detect} \cdot p_{correct} \cdot p_{coverage} \cdot R - (1-p_{detect}) \cdot P  -(1-p_{correct} + p_{correct} \cdot p_{not_coverage})\cdot p_{detect}\cdot P_{punishment_for_wrong_correction} =$ $p_{detect} \cdot p_{correct} \cdot p_{coverage} \cdot R - (1-p_{detect}) \cdot P  -(1 - p_{correct} \cdot p_{coverage})\cdot p_{detect}\cdot P_{punishment_for_wrong_correction} < 0$ }, condition \ref{eq:reward} is even more
...
In the previous section we presented empirical assessment of the distribution of corrections to a sentence. We now turn to estimating the resulting bias, namely
...
errors introduced by the correctors described in \S \ref{sec:formal_conservatism} were
...
to correlate much better with human rankings. \newcite{sakaguchi2016reassessing} argue for emphasizing
fluency over conservatism in compiling reference annotations.
%to construct reference annotations that emphasize .
