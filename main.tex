...
and the effect of $M$ on commonly used RBMs (Section \ref{}), extending the results of
...
We abstract away from the details of the training procedure, and assumed that correctors
are tuned to maximize the evaluation measure, over some training or development data.

Assume the corrector is faced with a phrase (or sentence) which it predicts to be
...
a valid correction for this phrase. Finally, assume that the corrector is evaluated
...
such a corrector is thus dincentivized from predicting a correction.
We expect Condition \ref{eq:reward} to frequently hold, especially in cases that
...
likely to hold.
Asymmetric measures, such as F-score \cite{} or I-measure \cite{}, are the most common evaluation
...
This experiment is thus very similar to the analysis reported by \newcite{bryant2015far},
