...
\lc{doesn't that cover the same thing as the conclusion paragraph}\oa{it's a reference to the prev work section}
...
Addressing the need to improve GEC evaluation, we propose three sophisticated
measures have been proposed. $M^2$ that is the standard since it was introduced
for CoNLL2013, it uses an edit lattice to compute phrase-level edits $F$-Score.
Two recent measures have been proposed to overcome $M^2$ shortcomings. GLEU \cite{napoles2015ground} an n-gram based measure shown to correlate with human judgments inspired by BLEU score and I-measure \cite{felice2015towards} an improvement of token-level edits accuracy score scaling from -1 to 1 providing a way to know if a correction is an improvement over the source.
...
to different evaluation criteria of the output. We believe that in terms of coservatism,
...
for a high number of references.\oa{the conclusion was too long. please let me know if there's something
  I missed}
