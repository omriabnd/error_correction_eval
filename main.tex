...
annotators are often instructed to be conservative in their corrections.\lc{daniel answered this is true in some sense, is there a proper way to write it?}
A recent attempt to formally capture this precision/recall asymmetry consisted of shifting from the use of $F_1$
...
corpus of LL paragraphs and corrected versions of them which is the de
...
may be different, and third, source sentences may be split or concatenated in the reference.
We quantify these divergences using three corresponding measures.

In order to measure sentence splitting/concatenation, we aligned sentences and counted how many sentences were split and how many concatenated. To align we matched sequentially last words in sentences.

In order to measure word changes, we first compute word alignment, and then we count the number of different aligned words, in addition to the deleted or added words.
...
exist in both text units will have low edit distance. We consider
aligning two sets of words a bipartite graph matching problem, with
weights according to the edit distance. For tie breaking, we add a
penalty. The penalty is always smaller than the minimum cost of
...
In order measure word order differences, we compute the same alignment and use Spearman's rho, a way to assess how well the relationship between two variables can be described using a monotonic function. 
...
\paragraph{Results.}
Figures \ref{fig:split}, \ref{fig:words_changed}, \ref{fig:rho} present our results using the split and concatenate, number of words changed per sentence and Spearman's row
...
sentences, changes more words and makes more word order changes, often an order of magnitude
more. For example, in the gold standard there are 44 sentences which have 5 words corrected, the most sentences with only 4 corrections another corrector has is 34.
