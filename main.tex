...
Let $X$ be the evaluated set of source LL sentences where $X$ consists of the sentences $x_{1}\ldots x_N$, each independently sampled from some distribution $\mathcal{L}$ over LL sentences and denote $\mathcal{D}_{x_i}\coloneqq \mathcal{D}_{i}$.
...
or choose to correct it with the correction it finds the most likely. If it selected not to correct the sentence, its probability of being rewarded (i.e., producing
an output in the reference set $Y$) is $(1-p_{detect})$. Otherwise, its probability
...
Moreover, asymmetric measures (e.g. $F_{0.5}$) further penalize invalidly correcting more harshly than not correcting an ungrammatical sentence. Denoting that penalty $P$, $\left(1-p_{correct}p_{coverage})\right)P$
 is subtracted from the left-hand side of Condition \ref{eq:reward}, making it much more likely to hold.
\subsection{Data}
