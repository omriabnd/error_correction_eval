...
in addition to two of the best performing systems on this dataset, a hybrid corrector and a neural network based corrector.
...
to the learner. A priori, one might have thought the opposite would occur, more references would lead to more conservatism. Having more references would introduce, among others, more conservative references and as it is more likely correctors produce a correction similar to the source those ones will be chosen.
...
across values. {\sc UnseenEst} aims to minimize earthmover distance between the estimated histogram and the histogram of the distribution and was proven empirically and mathematically to be very exact. \footnote{ <to be disclosed upon publication>\com{\href{https://github.com/borgr/unseenest}}{UnseenEst implementation}} 
...
combining semantic and grammatical measures. Despite their similarity, all those methods did not use current semantic annotation schemes or had other specific reasons not to be used in our case.
...
systems we experiment on in \S\ref{sec:formal_conservatism} and \ref{sec:increase-reference} are over-conservative, they are unlikely
to produce semantically unfaithful sentences, we instead experiment on 5 partially 
trained correctors, provided by \newcite{sakaguchi2017grammatical}, on the
JFLEG corpus \cite{napoles2017jfleg}.

  {\sc UCCASim} was calculated automatically for each system's output on 754 sentences. We expected to have low results as indeed these outputs include major changes, sometimes deleting full phrases from the output or changing every other word in the sentence, they also receive a low score by GLUE. As a sanity check we ran {\sc UCCASim} over 4 references. The references had {\sc UCCASim} scores of 0.72-0.78, the corrector predicted (by GLUE score) to be worse got 0.19 and the rest 0.32-0.39. Showing that bad corrections, that change the meaning as represented by UCCA, are penalized. 
  \com{
  \begin{table*}[]
  	\centering	
...
\begin{table}[]
	...
	\end{tabular}
\end{table}
  In the example \ref{ex:sensitive} one could see the general trend, with the source getting a {\sc UCCASim} of 1 the reference 0.71 and the corrector 0.33. Moreover, even though the chosen reference changes more words, getting the verb structure wrong is enough for an overall lower score for the corrector's sentence.
