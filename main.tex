...
\begin{figure}
  ...
  \caption{Mean Accuracy and Exact Index Match
    values for a perfect corrector (y-axis) as a function of the number of references $M$ (x-axis).
...
{\color{red}Finally, Figures \ref{fig:accuracy_vals} include confidence intervals ($p=.95$), which
are computed analytically. }\lc{I need to take what I said back. \cite{hong2013computing} is indeed used for the PDF, but currently we don't publish anything that uses the pdf. We do compute the confidence intervals, but those are intervals of the mean, for the graph. The account for the significance of the means we got i.e. for the choices of different sentences which we can't compute analytically. So we bootstrap again. The variance (not specifically significance) of the estimator was originally discussed with the analytical formula ($\sum_{i=1}^{N}P_{\mathcal{D}_x}(y_i \in Y)\cdot\left(1-P_{\mathcal{D}_x}(y_i \in Y)\right)$) when what is interesting is actually just that it decreases with N and when M gets away from 0.5.
We can also compute using bootstrapping with 10000 repetitions }Specifically, we note that the accuracy measure is a Poisson
Binominal distribution (i.e., a sum of independent Bernoulli variables with different
success probabilities), whose success probabilities are $P_{y,Y \sim \mathcal{D}_i}(y \in Y)$,
which we compute, as before, using {\sc UnseenEst}'s estimate for $\mathcal{D}_i$.
We use bootstrapping to compute intervals (accounting for sentence variability).
...
In order to obtain comparable results to those obtained when experimenting on the NUCLE
corpus (see below), we take $N=1312$, as in NUCLE's test set,
and further assume that 136 of these sentences require no correction.
These sentences reduce the overall bias by their frequency in the corpus (in our
case by 10.4\%), and are thus important to include for obtaining comparable results.
...
that our results would generalize to other reference-based measures, such as the\lc{doesn't that cover the same thing as the conclusion paragraph}
{\color{red}GLEU \cite{napoles2015ground} and I-measure \cite{felice2015towards}, which are adaptations of the BLEU and accuracy measures, respectively, that address orthogonal shortcomings from the ones presented here.
}
...
measure of fluency (e.g., \cite{sakaguchi2016reassessing}), as it only captures
the faithfulness dimension, namely the extent to which
the meaning of the source is preserved in the correction,
and not the correction's grammaticality.\lc{The reduction to semantic and error detection is another related way to convert this into an evaluation measure, one without references}
...
in $G_1$ and $G_2$ by $|E_1|$ and $|E_2|$, respectively. Their {\it DAG $F$-score} is their harmonic mean.
...
\begin{table}[h!]
  ...
  \begin{tabular}{c|c|c|c||c|c|}
    ...
    \hline
  \end{tabular}
  \caption{On the left {\sc UCCASim} with mapping from source to reference, the opposite and their average. On the right {\sc DistSim} for adverbs and participants and for scenes. The rows show values when the source and reference were annotated by the same annotator or by different ones. As baseline we use IAA and the translation between English and French. The correction similarity introduces less noise than the different annotators and annotating LL changes even less than translating does.\label{tab:Distances}}
\end{table}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusion}
Answering the need for improving evaluation for GEC three sophisticated measures have been proposed. $M^2$ that is the standard since it was introduced for CoNLL2013, it uses an edit lattice to compute phrase-level edits $F$-Score. Two recent measures have been proposed to overcome $M^2$ shortcomings. GLEU \cite{napoles2015ground} an n-gram based measure shown to correlate with human judgments inspired by BLEU score and I-measure \cite{felice2015towards} an improvement of token-level edits accuracy score scaling from -1 to 1 providing a way to know if a correction is an improvement over the source.
