  ...
  \caption{\label{fig:over-conservatism}
    ...
    of the systems.}
  	...
  	\begin{figure}[tbp]
  		...
  		\begin{subfigure}[]{0.4\textwidth}
  			\com{\caption{\label{fig:words_changed}}}
  			\includegraphics[width = \textwidth]{words_differences_heat}
  		...
  		\caption{(Top) Number of source sentences (y-axis) split 
  			(right bars) or concatenated (left bars) in the gold standard (striped column) and in different correctors (colored columns). The gold standard makes about an order of magnitude more splits and concatenations than the correctors.\\
  			...
  			(Bottom) Amount of sentences(heat) by number of words changed(x-axis) per system(y-label). The gold standard(bottom) corrects more words per sentences and more sentences than the correctors.\\
  			See \S\ref{par:experimental_setup} for a legend
  			of the systems.\label{fig:over-conservatism}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		By the estimates from {\sc UnseenEst} most sentences have a large number of corrections with low probability accounting for a major part of the probability mass and a rather small number of frequent corrections. Specifically, the distributions tend to have steps with many corrections with the same (low) frequency. In table \ref{tab:corrections_dist} different mean numbers of variants and their mass can be seen, when ignoring variants with negligible frequency.
		\begin{table}[h!]
			\centering
			\singlespacing
			\begin{tabular}{c|c|c|c|c|}
				\cline{2-5} 
				& \multicolumn{4}{c|}{Frequency Threshold}\\ 
				%\cline{2-5} 
			...
			\end{tabular}
			\caption{Mean number of variants which are not negligible by different minimum frequency per variant as a threshold and their mass.\label{tab:corrections_dist}}
		\end{table}
		...
		measure of a corrector, it is a basic, interpretable measure and a variant of it was recently suggested as evaluation \cite{felice2015towards}\lc{added one of the two newly found evaluation methods it helps our claim that accuracy is an interesting measure.
		...
		\begin{equation*}
		Acc\left(C;X,Y\right) = \frac{1}{N} \sum_{i=1}^n \mathds{1}_{C\left(x_i\right) \in Y_i}
		\end{equation*}
		...
		%to the source. 
                ...
                is then
\lc{make all equations footnotesize and make this one aligned}
                \begin{footnotesize}
                  \centering
                \begin{flalign*}
                ...
                \left(1 - Pr\left(C\left(x\right) \in Y \vert C\left(x\right) \in Correct_x\right) \right)&
                \end{flalign*}
                \end{footnotesize}

                
		It is easy to see that the bias is not affected by $N$, only by $M$, and we therefore denote it
                with $b_M\left(C\right)$. As $M$ grows, $Y$ becomes a better approximation of $Correct_x$, and $b_M$ tends to 0.
		...
		Formally, we assume that $Pr\left(C\left(x\right) \in Y \vert C\left(x\right) \in Correct_x\right) = Pr_{\mathcal{D}_x}\left(y \in Y\right)$.
		Under this assumption $b_M\left(C\right)$ can be written as the true accuracy of $C$ multiplied
		...
		our graphs by that accuracy. Similarly, assuming only a certain percentage of the sentences need correction will also scale the graph by that percentage. Denote the bias of a perfect corrector with $b_M$. To recap:
		\begin{equation*}
		b_M = 1 - Pr_{x \sim L, Y \in \mathcal{D}_x^M, y \sim \mathcal{D}_x}\left(y \in Y\right)
		\end{equation*}
		...
		%with probabilities $p_i = Pr_{y \sim \mathcal{D}_i}\left(y \in Y_i\right)$.
                ...
                \begin{footnotesize}
		\begin{equation*}
		\hat{b}_M = 1 - \frac{1}{N}\sum_{i=1}^N Pr_{Y \sim \mathcal{D}_i^M, y \sim \mathcal{D}_i}\left(y \in Y\right)
		\end{equation*}
                \end{footnotesize}
		...
		sampling. Specifically, for every $M\in[20]$ and $x_i$, we sample $Y_i$ 1000 times,
                with replacement, and estimate $Pr\left(y \in Y_i\right)$ as the covered probability mass
                $Pr_{\mathcal{D}_i}[\{y: y \in Y_i\}]$.\footnote{We repeat these experiments where $Y_i$
                  is sampled without replacement, obtaining similar trends with faster increase in accuracy.}
		...
		We also experiment with a more relaxed measure, {\it Exact Index Match}, which is only sensitive
                ...
                if their word alignments with the source (computed as above) $a:\{1,...,|x|\} \rightarrow \{1,...,\left|y\right|,Null\}$
                and $a':\{1,...,\left|x\right|\} \rightarrow \{1,...,\left|y'\right|,Null\}$, it holds that 
                $y_{a\left(i\right)} \neq x_{i}$ iff $y'_{a'\left(i\right)} \neq x_{i}$ ($y_{NULL}$ and $y'_{NULL}$ are empty).
		...
		\subsection{Significance of Real-World Correctors}
                ...
                So far we have discussed the mean values of the assessment. However, since measures are often used as
                means of comparison between different systems, not less important than the bias is the
                variance of these estimators, and the significance level they yield.

                Concretely, we compute the confidence interval of different correctors with the current NUCLE
                test data ($M=2$), based on the same process as in Section \ref{subsec:Assessment-values}.
		...
		Figure \ref{fig:F_correctors} what presents our results, which presents a mixed picture: some
		...
		Nevertheless, it seems that the values of $M$ taken in NUCLE are sufficiently high to generally obtain statistical
                significance between different systems, despite the low value of $M$.
                %Figure \ref{fig:F_Ms} suggests that no substaintial improvement of significance is expected
                %expected in the transition from $M=2$ to $M=5,8$.
...
various fields such as genetics\cite{ionita2010optimal}.
