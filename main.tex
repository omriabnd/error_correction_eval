...
\begin{abstract}
  ...
  we find that state-of-the-art systems make substantially less changes to the source sentences
  than needed.
...
{\color{red} In fact, it has been argued} that many would prefer a somewhat cumbersome
or even an occasionally ungrammatical
...
However, during development and training penalizing over-correction more harshly than under-correction may lead to reluctance of correctors to make any changes (henceforth, {\it over-conservatism}).
...
(\S \ref{sec:formal_conservatism}). {\red{color} While it may be possible that it is
beneficial for a corrector to be more conservatism than a human corrector would be,
we often find an order of magnitude differences between them, which are unlikely
to be desirable.}
...
{\color{red} Moreover, in \secref{sec:} we show that current correctors may already surpass
  human correctors in terms of common reference-based measures, suggesting that more substantial
  changes to the evaluation measure may be required.}
...
to the learner.\footnote{We do not see a reason why this reduction in conservatism may
  result of the setup in itself. In fact, oracle reranking may in some cases result
  in additional conservatism, as more references are used, some of them may be more
...
mean number of annotators who judged a correction to be valid, where sentences are grouped by the number of times they appear in the data (the gathered corrections), is given in Figure \ref{fig:validity_judgements}. 
...
A similar approach was also taken in the field of machine translation (MT), after showing multiple references are better for evaluation, but costly \cite{albrecht-hwa:2008:WMT,turian2006evaluation}.
There were attempts to create reference-less measures \cite{reeder2006measuring,albrecht2007regression,specia2009estimating,specia2010machine} and to estimate translation quality in terms of its adequacy (similar to semantic faithfulness) and fluency (similar to
grammaticality)
\newcite{banchs2015adequacy}.
Perhaps the most similar approach to ours was taken by the MEANT measure \newcite{lo2011meant}, which is based on a comparison of semantic role labeling structures on the source and target. Our decision to use UCCA in our experiments is motivated by its wider coverage of predicate types (as opposed to a focus only on verbal structures in MEANT). An elaborate comparison of SRL-based and UCCA-based approaches can be found in \newcite{birch2016hume}. We make no claim of course that UCCA is the best or the only semantic representation that can be used for defining a semantic faithfulness measure. Future work will experiment with a wider variety of semantic representations, both symbolic and distributional (e.g., \cite{cheng2015syntax}.)
