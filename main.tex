...
on the $F$-score RBM, the most commonly used measure for GEC. Their work focused on
...
it has no effect on the training and tuning of correctors.\oa{do we say what we do more?}
...
Results show that the reference corrections make changes to considerably more source sentences than any of the correctors, and within each changed sentence changes more words and makes more word order changes, often an order of magnitude more. For example, the reference has 44 sentences with 5 word changes, where the most sentences with 5 word changes by any corrector is 11.\oa{this is not an order of magnitude}
...
where $\alpha$ is ....
%the ratio between the penalty for 
%reward for validly correcting an error and the penalty
%for and $R$ the reward for valid ones
%and the re  Denoting that penalty  the success reward, 
The condition is more likely to hold in these cases.\oa{fix this}
...
in accuracy reaching above $0.7$ with $M=20$.\oa{we shouldn't say it's very high, what's
the actual number? does it reach saturation?}
...
still with $M=5$ it is less than 0.5\oa{put in M=10}.
