...
\title{Understanding Reference-based Evaluation for Grammatical Error Correction}
...
Given $x_1\ldots x_n,y_1,\ldots y_m$ e weight of each edge is the edit distance between the tokens.
...
corrections of a sentence. We now turn to the consequent bias (under-estimation) in the
estimation of a string-based similarity measure, for different values for $M$.
Our goal is to assess what value of $M$ would be a sweetspot, which is both practical
(as collecting more references is potentially costly) and reasonably biased.
...
\paragraph{Sentence-level Accuracy.} We begin by analyzing the sentence-level accuracy measure,
namely the number of sentences which were corrected in a way that exactly matches one of the
references (also called ``Exact Match''). While this score is less commonly used as an
evaluation measure (compared to the F-score discussed below), it is the commonly used loss
function for training statistical GEC systems\cite{rozovskaya2010training,chodorow2012problems}\oa{put a few references}. Further motivation
for exploring it is its interpretability.

Given a corrector $C$, a set of test sentences $X=\{x_1,...,x_n\}$ and their reference
corrections $Y_1,...,Y_n$, we denote with $p_i$ the probability that $y_i$\oa{this needs to be
  much more formal; let's discuss}
%For simplicity, assume a corrector has a constant probability $c$ to correct
%an ungrammatical sentence. Denote with $p_i$ the probability that the $i-th$
...
assess the similarity of a proposed correction to a reference,
and thus indirectly capture both criteria: references
are both faithful and conservative, as thus similarity to them is an indirect indication
of the faithfulness and conservatism of the proposed correction. However, we saw that
the variability of valid corrections may be prohibitively difficult to capture with a handful
of references.

In this section we explore a second possibility to assess the faithfulness dimension

if
the sentence matches a gold-standard reference, it is faithful by stipulation

if the
reference is not very different from the source, string similarity with the reference is
an indication 

can be thought
of as a proxy for assessing both criteria

As we saw in the previous sections,
using string similarity measures measure conservatism

placing too much emphasis 

of the original sentence. We may thus distinguish between two notions
of changes the source: formal and semantic, where high-quality corrections
are semantically {\it faithful} the source, 
which we call \textit{conservatism} and \textit{faithfulness}.
While any change in the original string would not be considered conservative, only
