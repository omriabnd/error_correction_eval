...
gold standard. It uses an edit lattice to optimistically choose edits for the reference that
will best match those of the references. Since it was introduced $M^2$ is the standard scorer for GEC. 
I-measure \cite{felice2015towards} was shown to have some wanted attributes that the $M^2$ scorer lacks.
...
edits accuracy score. The motivation for GLEU \cite{napoles2015ground} came from finding a negative
correlation between the two score mentioned. GLUE is an n-gram based measure inspired by BLEU, so it is
based on correction sentences rather than on edits. This score was show to correlate better with human
