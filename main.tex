...
Turk workers.\oa{say a bit more on the correction procedure. How long did it take?
  Did you apply any filtering? Anything interesting about the protocol you used?}
...
satisfying results.\oa{do you mean you conducted these exps? or did the authors?}
...
occuring with a probability of 0.01 or higher.\oa{maybe the graph to show is the total probability mass
  of the $k$ most frequent variants, where $k=1,2,3,4...$?}
...
is then\oa{could you align it to the left?}
...
annotator, and are thus not repeated. We find similar trends with faster
increase in accuracy.\oa{how much faster?}
...
and $a':\{1,...,\left|x\right|\} \rightarrow \{1,...,\left|c'\right|,Null\}$, it holds that 
...
Finally, Figures \ref{fig:accuracy_vals} include confidence intervals ($p=.95$), which
are computed analytically. Specifically, we note that the accuracy measure is a Poisson
...
We use the method of \newcite{hong2013computing} to efficiently compute the intervals.
...
\paragraph{}
While out experiments focus on the accuracy and F-score measures, but we expect
that our results would generalize to other reference-based measures, such as the
I-measure \cite{} and GLEU \cite{}, which are adaptations of the F-score and accuracy
measures, respectively, that address orthogonal shortcomings from the ones presented here.\oa{please verify!}
...
\begin{figure}
  ...
  \caption{
    Mean $F_{0.5}$ values for a perfect corrector (y-axis) as a function of the number of references $M$ (x-axis).
