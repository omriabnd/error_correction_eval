...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		\end{equation}
                ...
                \begin{equation}
		  \footnotesize
		  TrueAcc(C) = \mathbb{E}_{x\sim{L}}\left[Pr\left(C\left(x\right)\in Correct_i\right)\right]
		\end{equation}
		...
		%to the source. 
                ...
                \begin{align*}
                  ...
                  TrueAcc(C) - Pr_{x \sim L,Y \sim \mathcal{D}_x}(C(x) \in Y)
                \end{align*}
                  
		It is clear that the bias is not affected by $N$, only by $M$, and we therefore denote it
                with $b_M$. With $M$ tending to infinity, the bias is 0. However, as the results above show,
                this is generally impossible in GEC, as enumerating all valid corrections is prohibitively difficult.

                %%% UNTIL HERE %%%
                
                %The question 
                %It is possible to get a good estimate of $Acc(C;X,Correct_i)$
		%by taking a large enough sample from $Correct_i$, which is the common practice
                %in reference-based evaluation.
...
the meaning of the source is preserved in the correction.\lc{The reduction to semantic and error detection is another way to convert this into an evaluation measure, one without references}
...
Table \ref{tab:Distances} on its left presents the {\sc UCCASim} scores obtained by comparing the 
...
Our results indicate thatea perfect corrector areceive a faithfulness score comparable
...
Finally, Table \ref{tab:Distances} on its right presents the results of label distributional
...
\begin{table}[h!]
  ...
  \begin{tabular}{c|c|c|c||c|c|}
  	\cline{2-6} 
  	& \multicolumn{3}{c||}{UCCASim} & \multicolumn{2}{c|}{DistSim}\\ \cline{2-6}
    ...
    Baseline & 0.85 & 0.81 & 0.83 & 0.95 & 0.96
	  \end{tabular}
  \caption{Similarity measures for different mapping, their average and token analysis. For UCCASim the baseline is the score over two LL annotations and for DistSim the translation between English and French. For mapping the baseline one annotator was consistently considered the reference and the other the source. The correction similarity introduces less noise than the different annotators and annotating LL changes even less than translating does.\label{tab:Distances}}
\end{table}
