...
\maketitle
  ...
  %  since if systems are similarly biased, their
...
In this section we argue that there is no incentive to create systems that can correct types of errors that suffer from ICB, on the contrary, there is an incentive not to.
Through a motivating analysis we show low-coverage incentivizes under-prediction (\S \ref{subsec:motivating_analysis}), and show that modern correctors, which were shown to outscore humans are astonishingly conservative, suggesting the connection we show is worth solving.
...
\paragraph{Types.}
To wrap up the state of current correctors we analyze under-prediction per type using \citet{bryant-felice-briscoe:2017:Long} data. As a measure of overall conservatism tendency we look at the ratio between the mean number of corrections by the correctors and the references' one (see Appendix \ref{ap:types}). Assuming error types that consider an open class of corrections (e.g. add a missing verb) will have lower coverage we look at the conservatism of those error types. We find the split of 0.5 ratio splits between those types and closed class types almost perfectly. Punctuation errors are an unexpected exception, found to have a high conservatism of 0.2.
Surprisingly, we find that the frequency of the error is unable to explain which correction is under-predicted (Pearson \textbf{-}0.29 P=0.16).
