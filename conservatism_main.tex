...
\maketitle
  ...
  In this paper we revisit the ICB, and show it has more far-reaching implications than previously discussed. First, while we agree with \citet{bryant2015far} that a
  perfect corrector should receive a perfect score, we show that ICB does not merely scales system performance by a constant factor, but
  ...
  Indeed, in Section \ref{sec:real_world} we show that some existing systems outperform humans on $M^2$, applying much less changes to the source.
  ...
  in cases where many possible changes are valid, as necessarily only some of them are covered by the reference set. 
  We support our claim in three ways. 
  First, we find that GEC systems under-correct, often predicting an order of magnitude less changes to the input than a human annotator does.
  ...
  Third, using oracle re-ranking experiments, we find that increasing the number of references alleviates this effect.\oa{add references to relevant sections}

  Recent work has also attempted to address ICB by increasing the number of references (henceforth, $M$) \cite{}.
  In Section \ref{sec:increase-reference} we study the potential gain from increasing $M$,
  by assessing the distribution of corrections per sentence, and find this approach to be lacking, due
  to the vast number of valid corrections per sentence and their long-tailed distribution.
  ...
  %We conclude that the ICB cannot be overcome simply by increasing $M$.
  ...
  and observe that: (1) the distribution of valid references for a given sentence is long-tailed as well; 
  (2) common measures for TS dramatically under-estimate performance; 
  (3) oracle re-ranking with additional references alleviates this under-prediction.
  ...
  The ICB further jeopardizes the reliability of common validation experiments for RBMs,
...
We begin by formulating a methodology for studying the distribution of valid corrections for a sentence, and the effect it
has on RBMs.
We then proceed to discuss the relation between the references and the behaviour of RBMs.
...
we use crowdsourcing to obtain a sample from $\mathcal{D}_x$ (see Appendix  \ref{ap:crowd}).
...
As $M$ grows, $Y$ approximates better $Correct_x$, and $b_M$ tends to 0.
...
%As Exact Index Match can be interpreted as an accuracy measure for GED, this indicates that GED evaluation suffers from similar difficulties.
	...
	\vspace{-0.1cm}
\paragraph{$F$-Score.}\oa{add GLEU here, and say why we don't use I-measure}
...
In this section we have established that that (1) as correctors can surpass human performance on RBMs, re-scaling cannot
be used to overcome the ICB, and (2) as the distribution of valid corrections is long-tailed, 
...
The exact index match analysis also suggests that similar effects may be applicable in Grammatical Error Detection.
...
and to target errors, which only have few valid corrections.
We first show that low coverage can lead to under-correction (\S\ref{subsec:motivating_analysis}),
then show that modern correctors make far fewer corrections to the source, compared to humans (\S\ref{}).
In \S\ref{} we show that increasing the number of references can alleviate this effect.
Finally, we show that open-class error types are more likely to be under-corrected, than closed-class ones (\S\ref{}).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{-.1cm}
\subsection{Under-correction in GEC Systems}
\vspace{-.1cm}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Higher $M$ Alleviates Under-correction}
...
\subsection{Under-correction by Edit Types}

In this section we study the prevalance of under-correction according to edit types, finding that open-class types of errors
(such as replacing a word with another word) are more starkly under-corrected, than more closed class corrections.
...
and that this may have adverse effects on GEC performance.
...
We find that all edit types are under-predicted on average, but that the least under-predicted ones are mostly corrections that are usually
uniquely defined. For instance, the top quarter of error types includes orthographical errors, pluarity inflection of nouns, 
adjective inflections to superlative or comparative forms and determiner selection. The bottom quarter includes categories such as 
...
The only potential exception to this regularity is the punctuation selection type, which is under under-predicted in a ratio of 0.2, which places
...
is even slightly negatively correlated with the under-correction ratio (Pearson \textbf{-}0.29 P=0.16).
A possible explanation to this effect is thus the disincentive of GEC systems to correct error types, for which even correct
prediction is unlikely to be rewarded due to ICB.
...
%Another evidence that systematic penalties do occur in GEC can be seen in \citet{our corpus} work that found corrections of some edit types are penalized on average.
...
Our experiments show that ICB may account for this under-correction. Concretely, we show that
...
For example on the least conservative model, the neural one, with $M=1$ 50 sentences were not changed, while only 29 remained the same 
...
This paper argues that using a low-coverage reference set has adverse effects on the reliability
of reference-based evaluation in GEC and consequently on the incentives offered to GEC systems.
It further argues that these effects can not be overcome by re-scaling or increasing the number of
