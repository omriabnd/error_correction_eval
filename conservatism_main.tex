...
\maketitle
    ...
    Consequently, existing systems can obtain comparable or
		...
		Moreover, overcoming ICB by increasing the number of references is unfeasible, 
  ...
  Recent work has also attempted to address ICB by increasing the number of references 
  (henceforth, $M$) \cite{bryant2015far,sakaguchi2016reassessing}.\oa{missing ref}
...
\paragraph{GLEU.} We repeat the procedure using the mean GLEU sentence score (Figure \ref{fig:F_Ms}), shown to correlate better with human judgments \cite{napoles-sakaguchi-tetreault:2016:EMNLP2016}, results with the GLEU aggregated score are about $2\%$ higher with a similar plateau trend. This resembles the plateau observed by \citet{sakaguchi2016reassessing} which tested \emph{systems} mean GLEU score with \emph{fluency} references. Seeing only systems' scores they hypothesized that the plateau was due to covering the majority of possible references, which we now know is not the case.
...
(such as replacing a word with another word) are more starkly under-corrected, than closed-class corrections.
Evaluating with low coverage RBMs does not incentivize systems to address open-class errors (in fact, it disincentivizes them to). Therefore, even if ICB is not the cause for this effect, current evaluation procedures will perpetuate it.
...
As a measure of under-correction tendency, we take the ratio between the mean
 number of corrections 
produced by the systems and by the references. 
...
The only exception to this regularity is the punctuation selection type, which is found in the lower quarter. See Appendix \ref{ap:types} for the complete results.
