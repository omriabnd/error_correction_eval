...
We conclude that developing, tuning and training correctors against a larger number of valid corrections is likely to reduce over-conservatism.
...
\textcolor{red}{
%To put the discussion in a different light, 
Reframing the discussion, RBMs are often validated by their correlation with human judgments and an accurate estimate of the systems' true performance is deemed less important. However, our results show that developing correctors to improve such under-estimating measures leads to systematic biases.}
...
\begin{figure}
	...
	\caption{The amount of sentences (y-axis) with a given number of words changed (x-axis) relative to reranking with $M=1$, following oracle reranking with different $M$ values (column colors). All references are randomly sampled except BN column that contains \newcite{bryant2015far} 10 references.
