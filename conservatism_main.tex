...
As the differences in the prevalence of changes in the outputs and in the references are often an order of magnitude large, this gap is probably undesirable (\S\ref{sec:increase-reference}).
...
Supplementary  \ref{ap:abbr} lists system names and abbreviations.
...
\begin{figure*}
  ...
  \caption{\label{fig:over-conservatism}
    ...
    See Supplementary  \ref{ap:abbr} for a legend of the correctors.
...
we use crowdsourcing to obtain a sample from $\mathcal{D}_x$ (see Supplementary  \ref{ap:crowd} for details).
...
(details in Supplementary  \ref{ap:validity_judgements})
...
sampling. Specifically, for every $M = 1,...,20$ and $x_i$, we sample $Y_i$ 1000 times (with replacement), and estimate $P\left(y \in Y_i\right)$ as the covered probability mass $P_{\mathcal{D}_i}\{y: y \in Y_i\}$. Based on that we calculate the accuracy distribution and expectation (see Supplementary  \ref{ap:poibin}).
...
and to have a low edit distance from the source \cite{narayan2015unsupervised}.
...
\newcite{Xu-EtAl:2016:TACL}, applying {\sc UnseenEst} to estimate $\mathcal{D}_x$ (Supplementary  \ref{ap:crowd} for details).
...
This may support an implicit combination of references, but it also leads to an unintuitive situation: an output identical to a reference does not receive a perfect score, but rather it depends how similar it is to the other
references. A more in-depth analysis of SARI's handling of multiple references is found in Supplementary \ref{ap:sari-assum}.
...
with $M=8$. See Supplementary  \ref{ap:simp-rerank} for further details.
...
Second, we present a methodology for applying the UnseenEst technique, which to our knowledge hasn't been used
before in NLP, for assessing the distribution of target corrections. Our findings demonstrate how such
methodolical tools can provide insight as to the biases of existing systems and evaluation measures.
...
We show that the caveats of using low reference coverage are not a matter of scale.
Rather, low coverage leads to systematic biases, which has a neagative effect on system performance.

Our oracle re-ranking experiments suggest that incorporating additional references into the training and tuning protocol may reduce over-conservatism. 
