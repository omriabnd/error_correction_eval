...
\maketitle
	...
	\begin{abstract}
		The prevalant use of too few references for evaluating text-to-text
    ...
    Consequently, existing systems can obtain comparable or
		...
		We further find that overcoming ICB by increasing the number of reference is unfeasible, 
		due to long-tail distribution of valid corrections for a sentence.
	...
	A number of proposals for GEC are currently discussed in the literature, most of which are reference-based measures (RBM), including
	the $M^2$ measure, which is the de-facto standard, but also others such as GLEU and I-measure \cite{felice2015towards,napoles2015ground}.
  ...
  and concluded that reference-based measures (RBMs) under-estimate the performance of GEC systems.
  To address this, they proposed a new measure, Ratio Scoring (RS), which scales the common $M^2$ measure \cite{dahlmeier2012better}, 
  by the inter-annotator agreement, which can be construed as the score assigned to a perfect (human) corrector. 

  In this paper we revisit the ICB, and show that it has much graver effects than previously considered. First, while we agree with \citet{bryant2015far} that a
  perfect corrector should receive a score of 1, we show that the ICB does not consist in uniformly under-estimating performance by a constant factor, but
  rather that some correction policies are less prone to under-estimation. Concretely, we show that by only correcting errors where few possible
  corrections are valid, and refraining from correcting other cases, systems can outperform humans in RBMs. Indeed, we show that some existing systems outperform
  humans in the common $M^2$ measure, while making few changes to the output.\oa{add section references}
  
  We hypothesize that the development of GEC systems against a small number of references has resulted in a bias towards under-prediction of changes to the input
  where many possible corrections are valid. We provide several indications that this may be the case. First, we find that GEC systems are over-conservative,
  often predicting an order of magnitude less changes to the input than a human annotator does. Second, using oracle re-ranking experiments,
  we find that by increasing the number of references alleviates this under-prediction.
  Third, we find that the distribution of changes to the input made by examined systems tends more towards edit types that have a greater number of valid references.
  ...
  %\lc{what kind of new methodologies do we suggest and at 
  %which part should it appear? Refering to https://docs.google.com/document/d/1-Cdy9NHlh3RQhjMTjUuIIdQe17JSxh7AtDJs31loYSc/edit} 
  ...
  using the $M^2$ measure, RS, as well as on 0-1 loss and GLEU, finding diminishing returns. We conclude that ICB using current RBMs cannot be addressed by
  simply increasing $M$.
  ...
  (3) increasing $M$ with a proper measure alleviates this over-conservatism\lc{I think we should rephrase, lets call over-conservatism only the strictly 
  ...
  In recap, we find that the limited reference coverge in RBMs yields consistent biases in GEC evaluation, and consequently in 
  the GEC systems developed to optimize these biases. We further argue that these biases cannot be overcome by re-scaling nor by increasing the number
  of references in any feasible range. We note that the consistent biases in system performance also jeoperdize
  the reliability of human ranking experiments as a method for determining the validity of RBMs, since if systems are similarly biased, their
  system outputs is not representative of the space of valid system outputs.
  In Section \ref{}, we discuss paths for addressing these difficulties.
...
All these findings suggest that it is too costly to increase $M$ in development data to the extent asymmetric evaluation will seize to be influential. The exact index match analysis also suggests $M=1,2$ coverage is also low for detection. Different approaches that rely less heavily on the surface form of the reference seem to be the right direction to solve it. In recent ongoing work we suggest USim \citet{us} a reference-less evaluation measure which by definition does not suffer from this biases.
...
**copied from the analysis section
