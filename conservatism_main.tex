...
\maketitle
  ...
  The Inadequate Coverage Bias (ICB) was previously discussed by \citet{bryant2015far}, 
  ...
  In this paper we revisit the ICB, and show it has more far-reaching implications than previously discussed. 
  First, while we agree with \citet{bryant2015far} that a human correction should receive a perfect score, 
  we show that ICB does not merely scale system performance by a constant factor, but
  ...
  systems can outperform humans. 
  Indeed, in Section \ref{sec:real_world} we show that some existing systems outperform humans on $M^2$ and GLEU, 
  while only applying few changes to the source.
  
  We thus argue that the development of GEC systems against low coverage RBMs disincentivize systems from making changes to the source 
  in cases where many possible corrections are valid, as necessarily only some of them are covered by the reference set. 
  We support our claim by showing
  (1) that existing GEC systems under-correct, often performing an order of magnitude less corrections than a human does (\S\ref{subsec:under-correction});
  (2) that increasing the number of references alleviates under-correction (\S\ref{subsec:reranking});
  and (3) that under-correction is more expressed in error types that are more varied in their valid corrections (\S\ref{subsec:by_types}).

  Recent work has also attempted to address ICB by increasing the number of references (henceforth, $M$) \cite{}.\oa{missing ref}
  In Section \ref{sec:increase-reference} we estimate the distribution of corrections per sentence, 
  and find that increasing $M$ is unlikely to overcome ICB, due
  ...
  the score of a perfect corrector (i.e., the inter-annotator agreement) 
  ...
  (3) additional references alleviates this under-prediction.
  ...
  To recap, we find that the ICB in RBMs yields consistent biases in GEC evaluation, and consequently
  in systems developed to optimize such measures, and that these biases cannot be overcome by re-scaling or
...
Finally, we compare human and systems' performance with common RBMs (\S\ref{sec:real_world}).
\oa{legends in figure 1 are really small}
...
Appendix  \ref{ap:abbr} lists system names and abbreviations.\oa{re-order appendices}
...
variants a gene might have, including undiscovered ones, a setting similar to the one tackled here.
...
distributions can be found in <to be disclosed upon publication>\com{\href{https://github.com/borgr/unseenest}}.}
...
After estimating the histogram of valid corrections for a sentence, we turn to estimating the resulting bias (ICB), for different $M$ values. 
...
\begin{figure*}
	...
	\end{subfigure}
	\vspace{-0.2cm}
	\caption{Analysis (left) and empirical experiments (mid, right) to measure values for a perfect corrector (y-axis)
		as a function of the number of references $M$ (x-axis).For empirical experiments points are paired with a confidence interval ($p=.95$).}
	\vspace{-0.5cm}
\end{figure*}

Figure \ref{fig:accuracy_vals} presents the expected accuracy of a perfect
corrector (i.e., 1-$\hat{b}_M$) for different  $M$s. 
...
$M^2$ \cite{dahlmeier2012better} is the standard $F$-score scorer for GEC.
...
\paragraph{GLEU.} We compute the score of a perfect corrector according to the GLEU
measure, using the same bootstrap procedure we used for $F$-score. Results (Figure \ref{})
show similar trends.
...
\paragraph{}
We do not conduct similar experiments with {\sc I-measure},
as its run-time complexity grows exponentially with the number of references (due to
the way it mixes references), which renders it prohibitively costly for our methodology
that requires many applications of the measure.
...
Considering the $F$-score of the best-performing systems in Figure \ref{fig:F_correctors}, and comparing 
them to the $F$-score of a perfect corrector with $M=2$,\oa{why M=2? is this because that was used in CoNLL?}
we find that their scores are comparable, where the systems RoRo and JMGR surpass a perfect corrector's $F$-score.
Similar experiments conducted with GLEU show that these two systems obtain comparable or superior performance to humans
...
In this section we have established that (1) as correctors can surpass human performance on RBMs, re-scaling cannot
be used to overcome the ICB, and that (2) as the distribution of valid corrections is long-tailed, 
the number of references needed for reliable RBMs in exceedingly high.
...
Our analysis with Exact Index Match suggests that similar effects are applicable in Grammatical Error Detection as well.
...
%may be more effective in addressing ICB. 
...
In this section we demonstrate the adverse effects of ICB not only to the reliability of RBMs, but to the performance of GEC systems.
We argue that evaluation with inadequate reference coverage incentivizes systems to under-correct,
and to mostly target errors that have few valid corrections.
We first show that low coverage can lead to under-correction (\S\ref{subsec:motivating_analysis}),
then show that modern correctors make far fewer corrections to the source, compared to humans (\S\ref{subsec:under-correction}).
\S\ref{subsec:reranking} shows that increasing the number of references can alleviate this effect.
Finally, \S\ref{subsec:by_types} shows that open-class error types are more likely to be under-corrected, than closed-class ones.
...
that correctors attempt to maximize an objective function, 
...
Precision-oriented measures (e.g., $F_{0.5}$) penalize invalid corrections more
...
In this section we compare the prevalence of changes made to the source by the correctors,
...
To focus the evaluation on substantial changes, all non-alphanumeric characters were 
excluded, both within tokens or as tokens of their own. See Table \ref{tab:nucle_example} 
...
For example, there are 36 reference sentences with 6 word changes, where the highest number of sentences with 6 word changes by any corrector is 5.
A similar amount of corrections is observed on the references of the TreeBank of Learner English \cite{yannakoudakis2011new}.
...
\begin{figure}
	...
	where the amount for $M=1$ are subtracted from them.
...
In this section we study the prevalance of under-correction according to error types, finding that open-class types of errors
(such as replacing a content word with another) are more starkly under-corrected, than closed-class corrections.
These results further support our claim that ICB disincentivizes correcting errors where diverse corrections are possible, 
and that this may have adverse effects on the performance of GEC systems.
...
systems on the NUCLE test set.\oa{is this correct?}
As a measure of under-prediction tendency, we look at the ratio between the mean number of corrections 
producted by the systems and by the references. 
We note that this analysis does not consider whether the predicted correction is valid or not, but only how many of the errors of each
type the system attempted to correct. 

We find that all error types are under-predicted on average, but that the least under-predicted ones are of types that are often
uniquely defined. For instance, the top quarter of error types includes orthographical errors, pluarity inflection of nouns, 
adjective inflections to superlative and comparative forms and determiner selection. The bottom quarter includes  
verb selection, noun selection, particle/preposition selection, pronoun selection, and the type {\sc OTHER}, which is a residual category for error types.
The only potential exception to this regularity is the punctuation selection type, which is under-predicted in a ratio of 0.2, which places
it in the lower quarter. See Appendix \ref{ap:types} for the complete results.

We note that it is not the case that more frequent error types are the least under-predicted, as error type frequency
is in fact slightly negatively correlated with the under-correction ratio ($\rho$=-0.29 p-value=0.16).
A probable contributor to this effect is the disincentive of GEC systems to correct error types for which even valid
corrections are unlikely to be rewarded due to ICB.



%Assuming error types that consider an open class of corrections (e.g. add a missing verb) will have lower coverage we look at the conservatism of those error types. 
%We find the split of 0.5 ratio splits between those types and closed class types almost perfectly. Punctuation errors are an unexpected exception, found to have a high conservatism of 0.2.
...
%Another evidence that systematic penalties do occur in GEC can be seen in \citet{our corpus} work that found corrections of some types are penalized on average.
...
%\lc{should we add here a discussion of correlation to human judgments? should it be in a new subsection? section? paragraph in the discussion? we are out of space anyway}
...
has already been shown by previous work \cite{alvamanchego-EtAl:2017:I17-1}, 
which reported that TS systems tend to under-predict word additions, deletions,
...
Our experiments show that ICB may account for this under-prediction. Concretely, we show that
...
\newcite{Xu-EtAl:2016:TACL}, applying {\sc UnseenEst} to estimate $\mathcal{D}_x$ (Appendix  \ref{ap:crowd}).
...
identical to a reference does not receive a perfect score, but rather it depends on how similar it is to the other
references. A more in-depth analysis of SARI's handling of multiple references is found in Appendix \ref{ap:sari-assum}.
In order to neurtralize this effect of SARI, we also report the measure MAX-SARI, which coincides with SARI on $M=1$, 
...
with $k=100$, and one with a neural model \cite{nisioi2017exploring} with $k=12$. 
Our results indeed show that conservatism decreases with $M$ in both settings. 
For example, the least conservative model (the neural one) did not change 50 sentences with $M=1$, but only 29 weren't changed 
...
We argue that using a low-coverage reference set has adverse effects on the reliability
of reference-based evaluation in GEC and consequently on the incentives offered to GEC systems.
It further argues that these effects cannot be overcome by re-scaling or increasing the number of
references in a feasible way. 
The paper makes two methodological contributions to the GEC evaluation literature:
(1) a methodology for evaluating evaluation measures by the scores they assign a perfect corrector using a bootstrapping procedure;
...
We note that the ICB further jeopardizes the reliability of common validation experiments for RBMs,
that assess the correlation between human and measure rankings of system outputs.
Indeed, if outputs all under-correct, correlation studies will not be affected by whether an RBM is sensitive to under-correction. Therefore, RBMs tendency to reward such a policy will not be detected by such correlation experiments.


Our results underscore the importance of developing alternative evaluation measures that transcend $n$-gram overlap, 
and use deeper analysis tools, for instance by comparing
...
In a recent project,\footnote{Paper under review. Undisclosed to preserve anonymity.} we propose a complementary measure that 
