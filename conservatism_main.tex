...
\maketitle
    ...
    Consequently, existing systems can obtain comparable or
		...
		Moreover, overcoming LCB by increasing the number of references is unfeasible, 
  ...
  Concretely, we show that by only correcting closed class errors, where few possible corrections are valid, 
  and avoiding correcting others, systems can outperform humans. 
  ...
  in cases where many possible corrections are valid (open class errors), as necessarily only some of them are covered by the reference set. 
  To support our claim we show (1) that existing GEC systems under-correct, often performing an order of magnitude less corrections than a human does (\S\ref{subsec:under-correction});
  ...
  To recap, we find that the LCB in RBMs yields consistent biases in GEC evaluation and consequently
  in systems developed to optimize these measures, and that LCB cannot be overcome by re-scaling or
...
from the source. Since our crowdsourced corrections are annotated for edits, we produce edits heuristically.
...
which was shown to better correlate better with human judgments than $M^2$ \cite{napoles-sakaguchi-tetreault:2016:EMNLP2016}.
...
which we now know is not the case.\footnote{
We do not experiment with {\sc I-measure} \cite{felice2015towards}, 
...
them to the $F$-score of a perfect system with $M=2$,\oa{why M=2? is this because that was used in CoNLL?}
...
Assume the system is faced with a phrase which it predicts to be ungrammatical. 
...
Finally, assume the system is evaluated
...
Third, we report how many source sentences were split and how many concatenated by the reference and the systems.
...
examining the under-correction of the oracle re-ranker for different $M$ values, averaging over 1312 samples of 
...
indicating that under-correction is related to the low coverage of the reference set available for re-ranking.
...
Evaluating with low coverage RBMs does not incentivize systems to address open-class errors (in fact, it disincentivizes them to). Therefore, even if LCB is not the cause for this trend, current evaluation procedures will perpetuate it.
...
We note that this trend cannot be explained by assuming that common error types are targeted more often.
...
(3) under-prediction is alleviated with $M$ in an oracle re-ranking experiment.
...
In order to neurtralize this effect of SARI, we also report results with MAX-SARI, which coincides with SARI on $M=1$, 
...
with $k=100$, and with a neural model \cite{nisioi2017exploring} with $k=12$. 
...
We note that the LCB further jeopardizes the reliability of common validation experiments for RBMs,\oa{add references}
...
Therefore, the observed tendency of RBMs to reward under-correction cannot be detected by such correlation experiments.
