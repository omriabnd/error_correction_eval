...
\begin{table}[h]
	...
	\begin{tabular}{cc|cc}
		...
		& any S           & S is Correct                                                                              & S erroneous \\ \hline
	...
	\end{tabular}
	\caption{The different expected results under different assumptions about the ability of correctors to correct, the need for correcting most of the errors, and the probability to cover the correction with small $\mathcal{M}$. The values represent the probabilities keeping the source. $P$ is the probability to be in $Y$. \label{ta:oracle_expected_results}}
	\vspace{-0.9cm}
\end{table}
...
We also tested oracle reranking using SARI a somewhat precision oriented measure that is widely used. as a measure with 100-best list of moses based simplification system and 12-best of a neural based one, showing a tendency towards conservatism as the number of references is lower. For example neural system with $\mathcal{M}=1$ chose not to correct 7 sentences and made one edit to 60 of them while the one with $\mathcal{M}=8$ 3 and 31 respectively. In the same manner the Moses system with $\mathcal{M}=8$ has more sentences with $X$ or more corrections for every $X>1$. Graphs showing the improvement in word order and word changes can be found in Appendix \lc{add graphs}.
