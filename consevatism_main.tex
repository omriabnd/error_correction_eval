...
\begin{abstract}
  ...
  we show indications that this over-conservatism stems from
  the inability of a handful of references to account for the many valid
  corrections a sentence may have. This results in undue penalization of valid outputs,
  thus disincentivizing correctors to make changes.
  Increasing the number of references only partially addresses this gap,
...
(2) common measures for TS severely under-estimate human performance; 
...
may serve as an alternative to RBMs (\S\ref{sec:conclusion}). 
...
In order to see why a reduction in conservatism with an increase in $\mathcal{M}$ indicates
that tuning against a small set of references (low coverage) yields over-conservatism, assume we have an input sentence
with some sub-string $e$. 
There are three cases: (1) $e$ is an error, (2) $e$ is valid but there are valid references that alter it, (3) $e$ alone is valid. Case (3) can be ignored as oracle re-ranking does not have any effect in this case.
...
Whenever the reference allows both keeping $e$ altering $e$, the re-ranker conservatively selects keeping $e$ as it is. 
...
%In \S\ref{} and \ref{} we estimated the distribution of corrections that leads to low coverage and in turn may lead to the over-conservatism bias. We also had indications GED share a similar fate, although to a lesser extent. 
...
low coverage, taking SARI as an example RBM for TS;  
(3) the reduction in over-conservatism following oracle re-ranking with an increasing number of references.

We crowd-sourced 50 reference simplifications for 47 sentences, using the corpus and the annotation protocol presented by \newcite{Xu-EtAl:2016:TACL}, applying {\sc UnseenEst} to estimate their distribution.
...
Next we turn to assess the effect of $\mathcal{M}$ on SARI \cite{Xu-EtAl:2016:TACL}, a recently proposed but already widely-used measure. Most RBMs, Accuracy, $F$-score or BLEU maximize over the multiple references, unlike SARI that combines them. This may support an implicit combination of references, but it also leads to an unintuitive situation in which a simplification which is identical to a reference does not receive a perfect score. A more in-depth analysis of SARI's handling of multiple references can be found in Appendix \ref{ap:sari-assum}.
...
We also include the SARI score of a ``lucky perfect'' system, the randomly selects one of the given references (the MAX-SARI score for such a system is 1). Results show that SARI has a coverage of about $0.45$, and that this score is largely independent of $\mathcal{M}$.
...
lists of two systems, one with Moses \cite{elior2017summarization} with $k=100$, and one with a neural model \cite{nisioi2017exploring} with $k=12$. Our results
...
In light of these results, we expect these trends to generalize to any RBM that takes the source into account, and compares the changes made to it in the references and the system output. This includes most measures for monolingual translation, excluding multilingual originated ones.
