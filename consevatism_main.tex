...
\begin{abstract}
  ...
  Finally, we show that increasing the number of references only partially addresses this problem,
  and conclude by arguing for the use of reference-less measures as an alternative.
\end{abstract}
...
the correctors. Moreover, they show that increasing $\mathcal{M}$ alleviates the incurred bias, 
...
RBMs such as accuracy and $F$-score thus show diminishing returns from increasing the value of $\mathcal{M}$ over values of about 10.
...
While it is possible that these correctors performed this feat by learning not only how to correct, but also how to correspond to the writing style 
of one of NUCLE's annotators, this seems unlikely given that our conservatism analysis (\S\ref{sec:formal_conservatism}) showed that these systems considerably underpredict
edits relative to NUCLE's references.
A more likely explanation is that these correctors having learned to predict when not to correct.
...
Their paper differs from this one in that it focused   
on obtaining a more reliable estimate of system
performance and proposed to do so by normalizing their score
with the score of a human corrector.
However, while such normalization may yield more realistic performance estimates,
it has no effect on the training and tuning of correctors. Moreover,
we show that it already yields above perfect score for a system that hardly corrects.
Moreover, this paper includes estimates the distribution of corrections,
and evaluates a wider variety of methods, including text simplification methods.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Coping with the bias}\label{sec:referenceless}
