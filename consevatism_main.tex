...
the correctors. \textcolor{red}{ Moreover, they show that increasing $M$ only partially addresses
the incurred bias, as both RBMs' assessment almost triple with the increasing $M$ but it plateaus for values of 10--20, reaching about $60\%$.
...
it has no effect on the training and tuning of correctors. Moreover, we show that in some cases it already yields above perfect score for a system that hardly corrects.
}
...
been used in machine translation (MT) evaluation (e.g., \cite{banchs2015adequacy, lo2011meant}).
