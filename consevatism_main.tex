...
\begin{abstract}
  ...
  Through the test case of grammatical error correction (GEC) and a replication on \lc{should we mention GED?} simplification we investigate three aspects of monolingual translation, over-conservatism, the distributions of translations and the coverage of evaluation measures. We find that state-of-the-art systems make substantially fewer changes to the source sentences
  than needed.
  ...
  We also show that simply increasing the number of references is a partial remedy,
  \lc{should that be changed with the split?}and conclude by presenting an alternative reference-less approach based on semantic similarity.
...
Using only one or two reference corrections, a common practice in GEC, compounds this problem, as correctors are not only harshly penalized for making incorrect changes, but are often penalized for making {\bf correct} changes not found in the reference.
...
}\lc{all true, but should that be in the introduction, now that we have two fields in which some work has already been done on the one and some on the other?}
...
\lc{change the tone, suggesting that we propose semantic approaches when references are too costly and propose one for GEC}
...
We do not see a reason why this reduction in conservatism may result from the setup itself. In fact, oracle reranking will not show these tendency if even one of our assumptions does not hold. For simplicity, lets assume we only consider a specific error and collapse the corrections in the $k$-best list to three: $S$ keeping the source untouched, $VC \subset Correct$ the valid corrections and invalid ones. Then, aided by table \ref{ta:oracle_expected_results} we see that if over-conservatism is due to systems' lack of ability, if most choices not to correct are valid and hence probable to be found in some of the references or if coverage is not a problem, then we would expect no effect in the experiment. 
...
\begin{table}[]
	...
	\singlespacing
	\begin{tabular}{cc|cc}
		& $|VC| = 0$ & \multicolumn{2}{c}{$|VC| \neq 0$}                                                                                \\
		&            & $S \in Correct$                                                                              & $S \notin Correct$ \\ \hline
		\multicolumn{1}{c|}{Small $\mathcal{M}$} & 1          & \begin{tabular}[c]{@{}c@{}}$\Pr(S, \neg VC) +$\\  $0.5 \Pr(S,VC)$\end{tabular} & $\Pr(VC)$    \\
	...
	\end{tabular}
	\caption{The different expected results under different assumptions about the ability of correctors to correct, the need for correcting most of the errors, and the probability to cover the correction with small $\mathcal{M}$. In the cells are the probabilities for the source to be chosen. $\Pr$ is the probability to be in $Y$. \label{ta:oracle_expected_results}}
\end{table}

%may in some cases result in additional conservatism, because, as more references are used, some of them may be more similar to the source, thus increasing conservatism. Also, measures tend to be based on the best match, so if due to a mistake or a permissive annotator the source is found in some references, like the case in \cite{bryant2015far}, then as $\mathcal{M}$ rises the probability to find the source as a reference rises and we would expect reranking to result in over-conservatism.
